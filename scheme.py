...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        result = SPECIAL_FORMS[first](rest, env)  # define ...
    else:
        procedure = scheme_eval(first, env)  # (define (inner z x) (+ x (* y 2) (* z 3))) inner)
        args = rest.map(lambda operand: scheme_eval(operand, env))  # x=1 y=2
...
def apply_primitive(procedure, args_scheme_list, env):
    ...
    try:
        ...
        raise SchemeError("fail to apply {0} to {1}".format(procedure, args))
        # END Question 4
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("unfit arguments and val")
        check_formals(formals)
        child.bindings = self.bindings.copy()
        while formals:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN Question 5A    (define x 15)
        evl = scheme_eval(expressions.second.first, env)
        env.define(target, evl)
        return target
        # END Question 5A
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN Question 9A
        # (define (f x) (* x 2)) == (define f (lambda (x) (* x 2))
        # first remove the function name f
        # (define (outer-func x y) (define (inner z x) (+ x (* y 2) (* z 3))) inner)
        # -->Lambda(formal:x,y,  body:(define ...), env)
        pair = Pair(target.second, expressions.second)  # (x)(* x 2)
        env.define(target.first, do_lambda_form(pair, env))
        return target.first
        # END Question 9A
    else:
...
def do_if_form(expressions, env):
    """Evaluate an if form. (if (= 4 2) 'a 'b)"""
    check_form(expressions, 2, 3)
    # BEGIN Question 13
    predict = expressions.first
    val = expressions.second
    true_val = val.first
    if scheme_true(scheme_eval(predict, env)):
        return scheme_eval(true_val, env)
    if val.second:
        return scheme_eval(val.second.first, env)
    return okay
    # END Question 13


def do_and_form(expressions, env):
    """Evaluate a short-circuited and form. (and 4 5 (+ 3 3))"""
    # BEGIN Question 14B
    cur_val = True
    while expressions:
        cur_val = scheme_eval(expressions.first, env)
        if scheme_false(cur_val):
            return False
        expressions = expressions.second
    return cur_val
    # END Question 14B
...
def check_formals(formals):
    ...
    p_set = set()
    while formals:
        if not scheme_symbolp(formals.first) or formals.first in p_set:
            raise SchemeError("bad-formed parameter")
        p_set.add(formals.first)
        formals = formals.second
    # END Question 11B
